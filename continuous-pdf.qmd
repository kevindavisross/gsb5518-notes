...
ggplot(data = xddf,
       aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data=subset(xddf , x < 0.25),aes(ymax=y),ymin=0,
...
ggplot(data=xddf,
  ...
  ylab(expression(f[X](x))) +
#  ggtitle(paste("P(1<X<2.5) for X ~ Exponential(1)")) +
  geom_rect(data=xddf, mapping=aes(xmin=0.5-0.1/2, xmax=0.5+0.1/2,
                                   ymin=0, ymax=dexp(0.5, rate = 2)),
            color="orange", fill = "orange") +
    geom_rect(data=xddf, mapping=aes(xmin=1.0-0.1/2, xmax=1.0+0.1/2,
                                   ymin=0, ymax=dexp(1.0, rate = 2)),
