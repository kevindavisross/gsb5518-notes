...
ggplot(z_box_prob,
  ...
  coord_fixed() +
  theme_classic() +
  geom_vline(xintercept = c(-0.5, 1.5), linetype = "solid", 
                 color = c("#CC79A7", "#E69F00"), size = 2, alpha = 0.5)


ggplot(data = data.frame(u = c(-3, 3)),
       mapping = aes(x = u)) +
  stat_function(mapping = aes(colour = "x = 1.5"),
                fun = dnorm,
                args = list(mean = 0 + 0.7 * (1.5 - 0),
                            sd = 1 * sqrt(1 - 0.7 ^ 2)),
                size = 1.2) +
  stat_function(mapping = aes(colour = "x = -0.5"),
                fun = dnorm,
                args = list(mean = 0 + 0.7 * (-0.5 - 0),
                            sd = 1 * sqrt(1 - 0.7 ^ 2)),
                size = 1.2) +
...
ggplot(z_box_prob,
  ...
  geom_abline(intercept = 640 - 0.7 * 80 / 70 * 610, slope = 0.7 * 80 / 70, linetype = "solid", 
                 color = c("#CC79A7"), size = 2, alpha = 0.5) +
  geom_point(data = data.frame(x = xs, y = 640 + 0.7 * 80 / 70 * (xs - 610)),
             aes(x = x, y = y, z = 0),
